  ...
  <el-select v-else ref="selectRef" :model-value="multiple ? state.defaultValue : selectDefaultLabel"
    ...
    <template #empty>
      <div class="t-table-select__table" :style="{ width: tableWidth ? `${tableWidth}px` : '100%' }">
        ...
        <el-table ref="selectTable" :data="state.tableData" :class="{
          radioStyle: !multiple,
          highlightCurrentRow: isRadio,
          keyUpStyle: isKeyup
        }" highlight-current-row border :row-class-name="getRowClassName" :row-key="getRowKey"
          @row-click="rowClick" @cell-dblclick="cellDblclick" @selection-change="handlesSelectionChange"
          v-bind="$attrs">
...
const defaultSelect = (defaultSelectVal: any[]) => {
  if (props.multiple) {
    const multipleList = defaultSelectVal
      .map(val => state.tableData.find((row: { [x: string]: any }) => row[props.keywords.value] === val))
    ...
    const row = state.tableData.find((item: { [x: string]: any }) => item[props.keywords.value] === defaultSelectVal[0])
    if (row) {
  ...
  if (!props.multiple) {
    if (val) {
      ...
      const defaultIndex = tableData.findIndex(
        (item: { [x: string]: any }) => item[props.keywords.value] === selectDefaultLabel.value
      )
      if (defaultIndex !== -1) {
