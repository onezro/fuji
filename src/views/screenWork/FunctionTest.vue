<template>
  ...
  import {
    ...
    FTQueryDefectCode,
    FTToolOnline,
    OqcfunctionTefectProductRecord
  } from "@/api/scrApi";
  ...
  const badSubmit = () => {
    ...
    OqcfunctionTefectProductRecord({...badForm.value,tools:'A001-2402103001493',workstationName:'BICV-ZZYB-SB-YSJ-001'}).then((res: any) => {
      msgTitle.value = "";
      msgType.value = true;
      if (res.success) {
        badVisible.value = false;
  ...
  const getChange = () => {
    ...
    if (barCodeData.startsWith("BICV")) {
      ...
      if (stopsForm.value.result == "OK") {
        ...
        FTQueryDefectCode(stopsForm.value.containerName).then((res: any) => {
          //   console.log(res);
          if (!res.success) {
            msgTitle.value = res.msg;
            msgType.value = res.success;
