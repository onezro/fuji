<template>
    <div class="p-2">
        <el-card :body-style="{ padding: '8px' }">
            <div class="flex justify-between">
                ...
                <el-form ref="formRef" :model="getForm" :inline="true" label-width="auto" size="small">
                    <el-form-item :label="$t('iqctestItem.ProjectName')" class="mb-2">
            ...
            <el-table :data="tableData.slice(
                ...
                <el-table-column prop="ProjectName" :label="$t('iqctestItem.ProjectName')" />
                <el-table-column prop="Description" :label="$t('iqctestItem.Description')" />
                <el-table-column prop="UOMName" :label="$t('iqctestItem.UOMName')" />
                <el-table-column prop="StatusText" :label="$t('iqctestItem.StatusText')" />
                <el-table-column prop="CreateUser" :label="$t('iqctestItem.CreateUser')" />
                <el-table-column prop="CreateTime" :label="$t('iqctestItem.CreateTime')" width="150" />
                <el-table-column prop="UpdateUser" :label="$t('iqctestItem.UpdateUser')" />
                <el-table-column prop="UpdateTime" :label="$t('iqctestItem.UpdateTime')" width="150" />
        ...
        <el-dialog v-model="addVisible" title="添加" width="500" :append-to-body="true" :close-on-click-modal="false"
            ...
            <el-form ref="addFormRef" :model="addForm" label-width="auto">
                ...
                <el-form-item :label="$t('iqctestItem.UOMName')" prop="UOMName">
                    <el-select v-model="addForm.UomName" placeholder="请选择">
                        <el-option v-for="item in uomData" :key="item.uomname" :label="item.uomname"
                            :value="item.uomname" />

                    </el-select>
                </el-form-item>
                <el-form-item :label="$t('iqctestItem.StatusText')" prop="Status">
  ...
  <el-dialog v-model="editVisible" title="修改" width="500" :append-to-body="true" :close-on-click-modal="false"
            ...
            <el-form ref="editFormRef" :model="editForm" label-width="auto">
                ...
                <el-form-item :label="$t('iqctestItem.UOMName')" prop="UOMName">
                    <el-select v-model="editForm.UomName" placeholder="请选择">
                        <el-option v-for="item in uomData" :key="item.uomname" :label="item.uomname"
                            :value="item.uomname" />
...
import {
    ...
    GetProjectQuery,
    AyscProject,
} from "@/api/incomingManage/iqctestItem";
import {
    ref,
    watch,
    onBeforeMount,
...
const getGetUomData = () => {
    GetUomQuery({}).then((res: any) => {
        uomData.value = res.content;
    });
};
const openAdd = () => {
...
const handleEdit = (row: any) => { 
    editForm.value = { ...row, ExecutionType: "Update",
        Status: row.StatusText === "启用" ? 0 : 1,
        UomName: row.UOMName
     };
...
const handleAddConfirm = () => {
    AyscProject(addForm.value).then((res: any) => {
        addVisible.value = false;
        getData();
    });
};
...
const handleEditConfirm = () => {
    editForm.value.UpdateUser = userStore.getUserInfo;
    AyscProject(editForm.value).then((res: any) => {
        editVisible.value = false;
        getData();
    });
};
